Extended Infomax ICA Binary compiled for Windows 10 x64
Based on original source win32 Visual Studio 6 (2001) source by Enghoff (http://cnl.salk.edu/~enghoff/download1/)
Modified by Ernest Pedapati and Ellen Russo
ernest.pedapati [at] gmail [dot] com

Infomax ICA implemented here might be under a patent by the Salk Institute and any commercial application using this type of algorithm (or the recompiled binary files distributed here) should contact the Salk Institute patent office.

Please help maintain this repository.
